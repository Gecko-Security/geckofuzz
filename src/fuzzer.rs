...
/// A fuzzer that implements Gecko logic using LibAFL's [`Fuzzer`] trait
///
...
#[derive(Debug)]
pub struct GeckoFuzzer<VS, Loc, Addr, Out, CS, IS, F, IF, IFR, I, OF, S, OT, CI, SM>
where
...
impl<VS, Loc, Addr, Out, CS, IS, F, IF, IFR, I, OF, S, OT, CI, SM>
    GeckoFuzzer<VS, Loc, Addr, Out, CS, IS, F, IF, IFR, I, OF, S, OT, CI, SM>
where
    CS: Scheduler<State = S>,
    IS: Scheduler<State = InfantStateState<Loc, Addr, VS, CI>> + HasReportCorpus<InfantStateState<Loc, Addr, VS, CI>>,
    F: Feedback<S>,
...
impl<VS, Loc, Addr, Out, CS, IS, F, IF, IFR, I, OF, S, OT, CI, SM> UsesState
    for GeckoFuzzer<VS, Loc, Addr, Out, CS, IS, F, IF, IFR, I, OF, S, OT, CI, SM>
where
    CS: Scheduler<State = S>,
    IS: Scheduler<State = InfantStateState<Loc, Addr, VS, CI>> + HasReportCorpus<InfantStateState<Loc, Addr, VS, CI>>,
    F: Feedback<S>,
...
/// Implement fuzzer trait for GeckoFuzzer
...
// implement evaluator trait for GeckoFuzzer
