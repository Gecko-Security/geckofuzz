...
class Gecko:
    # initializes the object with attributes to handle the subprocesses and output
    def __init__(self):
        self.process = None
        self.out = None
        self.cancel = False

    # abstract methof to be implemented by subclasses
    def to_command(self):
        raise NotImplementedError("to_command not implemented")

    # starts the subprocesses with the command returned by 'to_command' storing output in a file
    def run(self):
        out = str(uuid.uuid4())
        outfile = open(out, "w")
    ...
    # cancels the running subprocesses
    def cancelit(self):
    ...
    # checks if the subprocesses is still runing
    def is_running(self):
        return self.process is not None and self.process.poll() is None

    # returns the output file identifier
    def get_idx(self):
        return self.out

    # retrieves and processes the output from the subprocess
    def get_output(self):
...
# inherits gecko
# initialize with additional attributes specific to on-chain tasks from a JSON configuration
# 
class GeckoOnchain(Gecko):
    path = "/bins/cli_onchain evm"
...
class GeckoOffChin(Gecko):
    path = "/bins/cli_offchain evm"
